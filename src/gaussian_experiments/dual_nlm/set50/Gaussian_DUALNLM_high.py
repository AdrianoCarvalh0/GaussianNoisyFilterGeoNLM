...
def NLM_KL(noised, h, f, t):
	...
	matriz_m = NLM(noised, 70, 4, 7)		# default = 70, testei 60 em alguns [50, 55, 60, 65, 70]
...
def generate_gaussian_experiment_high_dual_nlm(parameters):
    """
    ...
    name_pickle_dual_nlm_output_high = parameters['name_pickle_dual_nlm_output_high']
    name_results_xlsx_dual_nlm_output_high = parameters['name_results_xlsx_dual_nlm_output_high']
    pickle_results_summary_high = parameters['pickle_results_summary_high']
    ...
    array_dual_nln_high_filtereds = []

    vector = load_pickle('array_pickle_nlm', pickle_results_summary_high)
    ...
    for vect in vector:
        ...
        array_dual_nln_high_filtereds.append(dct)

    # Save all NLM results (for all images) to a pickle file
    save_pickle(array_dual_nln_high_filtereds, dir_out_results, name_pickle_dual_nlm_output_high)

    # Export all results (NLM, GEO-NLM, BM3D) to an XLSX spreadsheet
    save_results_to_xlsx(       
        dir_out_results,
        name_results_xlsx_dual_nlm_output_high
    )
