...
def generate_gaussian_experiment_low_dual_nlm(parameters):
    """
    ...
    name_pickle_dual_nlm_output_low = parameters['name_pickle_dual_nlm_output_low']
    name_results_xlsx_dual_nlm_output_low = parameters['name_results_xlsx_dual_nlm_output_low']
    ...
    for vect in vector[7:]:
        ...
        array_dual_nln_low_filtereds.append(dct)

    # Save all NLM results (for all images) to a pickle file
    save_pickle(array_dual_nln_low_filtereds, dir_out_results, name_pickle_dual_nlm_output_low)


    # Export all results (NLM, GEO-NLM, BM3D) to an XLSX spreadsheet
    save_results_to_xlsx(       
        dir_out_results,
        name_results_xlsx_dual_nlm_output_low
    )
