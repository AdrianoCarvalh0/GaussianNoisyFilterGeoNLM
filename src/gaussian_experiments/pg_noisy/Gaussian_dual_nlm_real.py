...
def generate_gaussian_dual_nlm_experiment_real(parameters):
	"""
	...
	dir_out_dual_nlm = parameters['dir_out_dual_nlm']
	dir_out_results = parameters['dir_out_results']
	name_pickle_dual_nlm_output_real = parameters['name_pickle_dual_nlm_output_real']
	name_pickle_results_dual_nlm_output_real = parameters['name_pickle_results_dual_nlm_output_real']
	name_results_xlsx_dual_nlm_output_real = parameters['name_results_xlsx_dual_nlm_output_real']
	f = parameters['f']        # Patch radius (NLM )
	t = parameters['t']        # Search window radius (NLM )
	h = parameters['h']        # Filtering parameter 
	...
	for file in array_img_originals:
		...
		img_noisse_gaussian_np = skimage.io.imread(f'{dir_noisy_real_images}/{file_name}')  
		...
		array_dual_nln_real_filtereds.append(dct)

	# Save all NLM results (for all images) to a pickle file
	save_pickle(array_dual_nln_real_filtereds, dir_out_results, name_pickle_dual_nlm_output_real)

	# Export all results (NLM, GEO-NLM, BM3D) to an XLSX spreadsheet
	save_results_to_xlsx(
		dir_out_results,
		name_results_xlsx_dual_nlm_output_real
	)
